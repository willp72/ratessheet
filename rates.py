...
def fetch_flagstone_pages(url: str, max_pages: int = FLAGSTONE_MAX_PAGES) -> list[str]:
    ...
    with sync_playwright() as p:
        ...
        # Click page numbers by their label. Class names here are generated by
        # styled-components and change on every deploy, so they are useless as
        # selectors; the digits on the buttons are stable. The visible window
        # slides as you advance, so the next number stays reachable.
